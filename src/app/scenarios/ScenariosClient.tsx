    ...
    <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
      ...
      <div className="space-y-6">
        ...
        <ChartContainer
          ...
          subtitle={
            chartTab === 'fundedRatio'
              ? `Historical, scenario, and the fund\u2019s own 2024 AV baseline through ${targetYear}`
          ...
          explainer={
            chartTab === 'fundedRatio'
              ? `This chart overlays three series: ${latest.fy - ts.observations[0].fy + 1} years of historical market-basis funded ratio (solid), the forward projection under the current scenario assumptions (dashed, fund-colored), and the fund actuary's own baseline from the 2024 actuarial valuation (dotted grey). The actuary baseline assumes the statutory funding schedule is followed and all assumptions are met.`
              : `Historical employer contributions (solid) alongside the forward projection under your scenario (dashed) and the fund actuary's own 2024 AV baseline (dotted grey). The dashed line is what the city would pay each year under your assumptions; the cumulative figure in the subtitle is the total bill through the target year.`
          }
          source="Public Plans Database + 2024 actuarial valuations + scenario engine"
        >
          ...
          <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-600">
            ...
            {ts.projectionsBaseline && ts.projectionsBaseline.length > 0 && (
              <LegendSwatch color="#64748b" kind="dotted" label="Actuary baseline (2024 AV)" />
            )}
          </div>
          {chartTab === 'fundedRatio' ? (
            <ProjectionChart
        ...
        <section className="rounded-xl border border-slate-200 bg-amber-50 p-4 text-sm text-amber-900">
          ...
          fund&rsquo;s published 2024 actuarial valuation baseline. Scenario adjustments (return
